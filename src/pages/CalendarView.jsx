...
import { 
  ...
  Plus, X, Clock, BookOpen, FileText, Edit3, Trash2, Ban
} from 'lucide-react';
...
export default function CalendarView({ courses = [], coursework = [], setCoursework }) {
  ...
  const getEventsForDate = (date) => {
    ...
    // 1. 해당 날짜의 과제/일정 먼저 수집
    const dailyAssignments = coursework.filter(item => item.due_date && getLocalDateStr(item.due_date) === targetDateStr);
    
    // 🚀 휴강 및 시험 정보 파악 (수업 병합을 위해)
    const examCourseIds = new Set();
    const cancelledCourseIds = new Set();

    dailyAssignments.forEach(item => {
      const isExam = EXAM_KEYWORDS.some(kw => item.title.includes(kw));
      const isCancel = item.category === 'cancellation';
      const isPersonal = !item.course_id;

      if (isExam && item.course_id) examCourseIds.add(item.course_id);
      if (isCancel && item.course_id) cancelledCourseIds.add(item.course_id);
      ...
      if (item.course_id) {
        ...
        if (isExam) {
          type = 'exam';
          label = course ? `${course.name}-시험` : '시험';
        } else if (isCancel) {
          type = 'cancellation';
          label = course ? `${course.name}-휴강` : '휴강';
        } else {
          label = course ? course.name : '과제';
        }
      } else {
        type = 'personal';
        label = '일정';
      ...
      events.push({ 
        ...
        type, label, 
        title: item.title, 
        time: new Date(item.due_date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', hour12: false}), 
        isCompleted: item.is_completed, 
        raw: item 
      });
    });

    // 2. 해당 날짜의 수업 정보 추가 (시험/휴강이 있는 경우 제외하고 병합)
    courses.forEach(course => {
      if (cancelledCourseIds.has(course.id) || examCourseIds.has(course.id)) return;

      if (course.start_date && course.end_date) {
        const cStart = new Date(course.start_date); cStart.setHours(0,0,0,0);
        const cEnd = new Date(course.end_date); cEnd.setHours(23,59,59,999);
        if (date < cStart || date > cEnd) return;
      }
  ...
  const handleSaveAssignment = async (e) => {
    ...
    try {
      ...
      const payload = { 
        ...
        user_id: user.id,
        course_id: isPersonal ? null : assignForm.course_id,
  ...
  return (
    <div className="space-y-4 md:space-y-8 pb-24 text-left animate-in fade-in duration-500 px-1 font-sans">
      <div>
        ...
        <p className="text-gray-500 font-bold text-xs md:text-base mt-2">수업, 과제, 시험 및 휴강 일정을 스마트하게 관리하세요.</p>
      </div>
                  ...
                  {date && (
                    <>
                      ...
                      <div className="flex gap-[2px] mt-px h-[3px]">
                        ...
                        {hasExam && <div className="w-[3px] h-[3px] rounded-full bg-orange-500"></div>}
                        {hasPersonal && <div className="w-[3px] h-[3px] rounded-full bg-emerald-400"></div>}
                        {hasCancel && <div className="w-[3px] h-[3px] rounded-full bg-red-600"></div>}
                      </div>
                    </>
                  )}
                </div>
              ...
              {getEventsForDate(selectedDate).map((event) => (
                <div key={event.id} className="relative pl-3.5 py-0.5 flex flex-col justify-center">
                  <div className={`absolute left-0 top-0.5 bottom-0.5 w-[3px] md:w-1 rounded-full shadow-sm 
                    ${event.type === 'assignment' ? 'bg-rose-500' : event.type === 'personal' ? 'bg-emerald-400' : event.type === 'exam' ? 'bg-orange-500' : event.type === 'cancellation' ? 'bg-red-600' : 'bg-amber-400'}`}></div>
                  <div className="flex flex-col leading-tight">
                    ...
                    <span className={`text-[10px] md:text-[13px] font-bold truncate mt-0.5 ${event.isCompleted ? 'text-gray-500 line-through' : event.type === 'cancellation' ? 'text-red-400' : 'text-gray-100'}`}>
                      {event.title}
                    </span>
                  </div>
                </div>
      ...
      <div className="bg-white rounded-[28px] md:rounded-[32px] shadow-sm border border-gray-100 overflow-hidden mt-6">
        <div className="p-4 md:p-6 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50/50 border-b border-gray-100">
          ...
          <div className="flex flex-wrap justify-center items-center gap-3 md:gap-5 text-[11px] md:text-xs font-bold text-gray-500">
            <span className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-amber-400 shadow-sm"></div> 수업</span>
            <span className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-orange-500 shadow-sm"></div> 시험</span>
            <span className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-rose-500 shadow-sm"></div> 과제</span>
            <span className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-sm"></div> 일정</span>
          </div>
        </div>
                    ...
                    <div className="w-full flex flex-col space-y-[2px] overflow-hidden px-1 text-left">
                      {dateEvents.slice(0, 3).map(event => (
                        <div key={event.id} className="flex items-center gap-1 w-full truncate">
                          <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full shrink-0 ${event.type === 'assignment' ? 'bg-rose-500' : event.type === 'personal' ? 'bg-emerald-500' : event.type === 'exam' ? 'bg-orange-500' : event.type === 'cancellation' ? 'bg-red-600' : 'bg-amber-400'}`}></div>
                          <span className={`text-[9px] md:text-[11px] font-bold truncate ${event.type === 'assignment' ? 'text-rose-600' : event.type === 'personal' ? 'text-emerald-600' : event.type === 'exam' ? 'text-orange-600' : event.type === 'cancellation' ? 'text-red-600' : 'text-amber-700'}`}>
                            {event.title}
                          </span>
                        </div>
                      ))}
                      {dateEvents.length > 3 && <p className="text-[9px] md:text-[10px] text-gray-400 font-bold text-left pl-2.5 md:pl-3 pt-0.5">외 {dateEvents.length - 3}개</p>}
                    </div>
                  </>
                )}
              </div>
          ...
          <div className="bg-white rounded-t-[32px] md:rounded-[40px] w-full max-w-md p-8 shadow-2xl animate-in slide-in-from-bottom md:zoom-in-95 duration-300">
            <div className="flex justify-between items-start mb-8 text-left">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => openAddModal(selectedDate)} className="bg-indigo-50 text-indigo-600 px-3.5 py-2 rounded-xl font-black text-sm hover:bg-indigo-100 transition-colors shadow-sm">+ 일정 추가</button>
            ...
            <div className="space-y-3.5 max-h-[50vh] overflow-y-auto pr-2 pb-4 scrollbar-hide">
              {getEventsForDate(selectedDate).map(event => (
                <div key={event.id} className={`flex items-center p-4 md:p-5 rounded-[24px] border-2 transition-all ${event.type === 'assignment' ? 'bg-rose-50/40 border-rose-100' : event.type === 'personal' ? 'bg-emerald-50/40 border-emerald-100' : event.type === 'exam' ? 'bg-orange-50/40 border-orange-100' : 'bg-amber-50/40 border-amber-100'}`}>
                  <div className={`p-3.5 rounded-2xl mr-4 shadow-sm ${event.type === 'assignment' ? 'bg-rose-500 text-white' : event.type === 'personal' ? 'bg-emerald-500 text-white' : event.type === 'exam' ? 'bg-orange-500 text-white' : 'bg-amber-400 text-white'}`}>
                    <BookOpen size={22} />
                  </div>
                  <div className="min-w-0 flex-1 text-left">
                    <p className={`text-[10px] font-black uppercase tracking-widest ${event.type === 'assignment' ? 'text-rose-500' : event.type === 'personal' ? 'text-emerald-500' : event.type === 'exam' ? 'text-orange-500' : 'text-amber-600'}`}>{event.label}</p>
                  ...
                  {event.type !== 'course' && (
                    <button onClick={() => openEditModal(event.raw)} className="ml-2 p-2 bg-white text-gray-400 rounded-xl hover:text-indigo-600 hover:shadow-sm border border-transparent hover:border-gray-100 transition-all"><Edit3 size={18} /></button>
                  )}
                </div>
              ))}
            </div>
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-black/40 z-[120] flex items-center justify-center p-4 backdrop-blur-sm">
          <form onSubmit={handleSaveAssignment} className="bg-white rounded-3xl shadow-2xl w-full max-w-[480px] animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-7 space-y-6 overflow-y-auto text-left">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2 col-span-2 md:col-span-1">
                  <label className="text-[13px] font-black text-gray-700 ml-1">과목 지정 (선택 시 과제로 포함)</label>
              ...
              <div className="space-y-2">
                <label className="text-[13px] font-black text-gray-700 ml-1">일정명 (제목에 '시험' 포함 시 주황색 표시)</label>
                <input required placeholder="예: 히브리어 시험" className="w-full px-4 py-3.5 bg-gray-50 border border-transparent focus:border-[#6b62ff] focus:bg-white rounded-[14px] outline-none text-sm text-gray-800 placeholder-gray-400 font-bold transition-all" value={assignForm.title} onChange={e => setAssignForm({...assignForm, title: e.target.value})} />
              </div>
              <div className="space-y-2">
