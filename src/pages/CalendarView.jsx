...
import { 
  ...
  Plus, X, Clock, BookOpen, FileText, Edit3, Trash2, Sparkles, Ban
} from 'lucide-react';
      ...
      <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border-b border-gray-50">
        <div className="flex items-center gap-4">
          <div className="flex bg-gray-100 p-1 rounded-xl items-center">
            <button onClick={() => setCurrentDate(new Date(currentDate.setFullYear(currentDate.getFullYear() - 1)))} className="p-1.5 hover:bg-white rounded-lg text-gray-400 transition-all"><ChevronsLeft size={18} /></button>
            <button onClick={() => setCurrentDate(new Date(currentDate.setMonth(currentDate.getMonth() - 1)))} className="p-1.5 hover:bg-white rounded-lg text-gray-600 transition-all"><ChevronLeft size={18} /></button>
            
            <h2 className="px-4 text-lg font-black text-gray-900 min-w-[120px] text-center">
              {currentDate.getFullYear()}년 {currentDate.getMonth() + 1}월
            </h2>

            <button onClick={() => setCurrentDate(new Date(currentDate.setMonth(currentDate.getMonth() + 1)))} className="p-1.5 hover:bg-white rounded-lg text-gray-600 transition-all"><ChevronRight size={18} /></button>
            <button onClick={() => setCurrentDate(new Date(currentDate.setFullYear(currentDate.getFullYear() + 1)))} className="p-1.5 hover:bg-white rounded-lg text-gray-400 transition-all"><ChevronsRight size={18} /></button>
          </div>
          <button onClick={() => setCurrentDate(new Date())} className="px-4 py-2 bg-slate-50 text-[12px] font-black text-[#6366f1] rounded-xl hover:bg-indigo-50 transition-colors">오늘</button>
        </div>
        <button 
        ...
        <div className="fixed inset-0 bg-black/50 z-[2000] flex items-center justify-center p-4 backdrop-blur-sm">
          <form onSubmit={handleSaveAssignment} className="bg-white w-full max-w-[480px] rounded-[32px] p-8 space-y-6 animate-in zoom-in-95 duration-200 shadow-2xl text-left">
            <div className="flex justify-between items-center border-b border-gray-50 pb-4">
              <h2 className="text-xl font-black text-gray-900">{editingAssignId ? '일정 수정' : '새 일정 등록'}</h2>
              <button type="button" onClick={() => setShowAssignModal(false)} className="text-gray-400 hover:text-gray-600 transition-colors"><X size={24} /></button>
            </div>

            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-black text-gray-400 ml-1">일시</label>
                ...
                <div className="space-y-1">
                  ...
                  <select 
                    className={`w-full px-4 py-3.5 rounded-2xl font-bold text-sm border-none outline-none ${assignForm.category === 'cancellation' ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-700'}`}
                    value={assignForm.category} 
                    onChange={e => setAssignForm({...assignForm, category: e.target.value})}
                  >
                    <option value="schedule">📅 일반 일정</option>
                    <option value="assignment">📝 과제 등록</option>
                    <option value="cancellation">🚨 휴강 설정</option>
                  </select>
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-black text-gray-400 ml-1">과목 연결 (미선택 시 개인일정)</label>
              ...
              <div className="space-y-1">
                ...
                <input required disabled={assignForm.category === 'cancellation'} placeholder={assignForm.category === 'cancellation' ? "휴강 설정 시 과목명이 제목이 됩니다." : "제목을 입력하세요."} className="w-full px-4 py-3.5 bg-gray-50 rounded-2xl font-bold text-sm border-none outline-none focus:ring-2 focus:ring-indigo-100 disabled:opacity-50" value={assignForm.title} onChange={e => setAssignForm({...assignForm, title: e.target.value})} />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-black text-gray-400 ml-1">상세 내용</label>
                <textarea className="w-full px-4 py-3.5 bg-gray-50 rounded-2xl font-bold text-sm border-none outline-none min-h-[100px] resize-none focus:ring-2 focus:ring-indigo-100 transition-all" value={assignForm.description} onChange={e => setAssignForm({...assignForm, description: e.target.value})} placeholder="일정 세부 사항을 기록하세요." />
              </div>

              <div className="space-y-3">
                <label className="text-xs font-black text-gray-700 ml-1 flex items-center gap-2"><Sparkles size={16} className="text-indigo-500" /> 체크리스트</label>
                {assignForm.sub_tasks?.map((task, idx) => (
                  <div key={idx} className="flex gap-2 animate-in slide-in-from-left-2">
                    <input className="flex-1 px-4 py-2.5 bg-gray-50 rounded-xl text-xs font-bold border-none outline-none focus:ring-2 focus:ring-indigo-50" value={task} onChange={(e) => {
                      const nt = [...assignForm.sub_tasks];
                      nt[idx] = e.target.value;
                      setAssignForm({...assignForm, sub_tasks: nt});
                    }} />
                    <button type="button" onClick={() => setAssignForm({...assignForm, sub_tasks: assignForm.sub_tasks.filter((_, i) => i !== idx)})} className="text-gray-300 hover:text-red-400 transition-colors"><Trash2 size={18}/></button>
                  </div>
                ))}
                <button type="button" onClick={() => setAssignForm({...assignForm, sub_tasks: [...(assignForm.sub_tasks || []), '']})} className="w-full py-3.5 border-2 border-dashed border-gray-100 text-gray-400 rounded-2xl text-[11px] font-black hover:bg-gray-50 transition-all">+ 세부 일정 항목 추가</button>
              </div>
            </div>

            <div className="flex gap-3 pt-4 border-t border-gray-50">
              {editingAssignId && (
                <button type="button" onClick={async () => { if(window.confirm('삭제할까요?')) { await supabase.from('assignments').delete().eq('id', editingAssignId); if(setCoursework) setCoursework(coursework.filter(a => a.id !== editingAssignId)); setShowAssignModal(false); } }} className="px-6 py-4 bg-red-50 text-red-500 rounded-2xl font-black text-sm hover:bg-red-100 transition-colors">삭제</button>
              )}
