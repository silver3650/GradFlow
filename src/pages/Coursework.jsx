...
import { 
  ...
  X, Edit3, Clock, Trash2, Sparkles, Ban, RefreshCw
} from 'lucide-react';
...
export default function Coursework({ courses = [], setCourses, coursework = [], setCoursework, showAlert }) {
  ...
  const initialCourseForm = { name: '', professor: '', day_of_week: '월요일', status: 'in_progress', start_time: '09:00', end_time: '10:30', start_date: '2026-03-02', end_date: '2026-06-20' };
  const initialAssignForm = { title: '', due_date: '', course_id: '', is_completed: false, description: '', sub_tasks: [''], category: 'assignment' };
  
  const [courseForm, setCourseForm] = useState(initialCourseForm);
  const [assignForm, setAssignForm] = useState(initialAssignForm);

  const getDDayValue = (date) => date ? new Date(date).getTime() : Infinity;
  ...
  const activeAssignmentCount = useMemo(() => {
    return safeCoursework.filter(a => !a.is_completed && a.course_id && (a.category === 'assignment' || !a.category)).length;
  }, [safeCoursework]);
  ...
  const sortedCourses = useMemo(() => {
    return safeCourses
      ...
      .sort((a, b) => {
        ...
        return (aTasks[0] ? getDDayValue(aTasks[0].due_date) : Infinity) - (bTasks[0] ? getDDayValue(bTasks[0].due_date) : Infinity);
      });
  ...
  const handleSaveCourse = async (e) => {
    ...
    const res = editingCourseId ? await supabase.from('courses').update(payload).eq('id', editingCourseId).select() : await supabase.from('courses').insert([payload]).select();
  ...
  const handleSaveAssignment = async (e) => {
    ...
    // 🚀 개인 일정 자동 분류 및 휴강 제목 처리
    if (!assignForm.course_id) finalCategory = 'schedule';
    else if (assignForm.category === 'cancellation') {
      const c = safeCourses.find(item => item.id === assignForm.course_id);
      finalTitle = `${c?.name}-휴강`;
    }

    const payload = { ...assignForm, title: finalTitle, category: finalCategory, user_id: user.id, due_date: new Date(assignForm.due_date).toISOString() };
    const res = editingAssignId ? await supabase.from('assignments').update(payload).eq('id', editingAssignId).select() : await supabase.from('assignments').insert([payload]).select();
  ...
  return (
    <div className="space-y-6 text-left pb-10">
      <div className="flex justify-between items-center px-1">
        ...
        <button onClick={() => { setEditingCourseId(null); setCourseForm(initialCourseForm); setShowCourseModal(true); }} className="bg-[#1a1f2c] text-white px-5 py-2.5 rounded-2xl text-xs font-black shadow-lg flex items-center shrink-0"><Plus size={18} className="mr-1" /> 과목 추가</button>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-[32px] p-6 shadow-sm">
        <h3 className="text-lg font-black text-gray-800 mb-6 flex items-center gap-2"><CalendarDays size={20} className="text-[#6366f1]" /> 현재 과제 상황 ({activeAssignmentCount})</h3>
        <div className="grid grid-cols-3 gap-3">
          {sortedCourses.map(c => {
            ...
            return (
              <div key={c.id} onClick={() => document.getElementById(`course-${c.id}`)?.scrollIntoView({behavior:'smooth'})} className="bg-slate-50/70 p-3 rounded-2xl border border-gray-100 min-w-0 cursor-pointer shadow-sm hover:border-indigo-200 transition-all">
                <span className="font-black text-gray-900 text-xs block truncate">{c.name} ({tasks.length})</span>
                {tasks[0] && <p className="text-[10px] font-black text-red-500 mt-1">{getDDayLabel(tasks[0].due_date)}</p>}
              </div>
            );
          })}
        </div>
            ...
            <div key={course.id} id={`course-${course.id}`} className="scroll-mt-24">
              <div className="bg-indigo-900 text-white px-6 py-5 rounded-t-[32px] flex justify-between items-center shadow-lg">
                <div className="min-w-0 pr-4">
                  ...
                  <p className="text-xs text-indigo-200/80 font-bold ml-11">{course.professor} ㅣ {course.day_of_week}</p>
                </div>
                ...
                {tasks.map(a => (
                  <div key={a.id} onClick={() => { setEditingAssignId(a.id); setAssignForm({...a, due_date: a.due_date?.slice(0,16)}); setShowAssignModal(true); }} className={`bg-slate-50/50 p-6 rounded-[32px] border-2 border-gray-50 cursor-pointer hover:border-indigo-100 transition-all ${a.category === 'cancellation' ? 'border-red-100 bg-red-50/30' : ''}`}>
                    <div className="flex justify-between items-center mb-3">
                      <span className={`text-[10px] font-black uppercase ${a.category === 'cancellation' ? 'text-red-500' : 'text-indigo-400'}`}>{a.category === 'cancellation' ? 'Ban 휴강' : 'Task'}</span>
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-black/40 z-[2000] flex items-center justify-center p-4 backdrop-blur-sm">
          <form onSubmit={handleSaveAssignment} className="bg-white w-full max-w-[480px] rounded-[32px] p-8 space-y-6 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center"><h2 className="text-xl font-black text-gray-900">상세 정보 등록</h2><button type="button" onClick={() => setShowAssignModal(false)}><X size={24} className="text-gray-300"/></button></div>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1"><label className="text-xs font-black text-gray-400">분류</label><select className={`w-full px-4 py-3 rounded-xl font-bold text-sm border-none outline-none ${assignForm.category === 'cancellation' ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-700'}`} value={assignForm.category} onChange={e => setAssignForm({...assignForm, category: e.target.value})}><option value="assignment">📝 과제 (건수 포함)</option><option value="schedule">📅 일정 (건수 제외)</option><option value="cancellation">🚨 휴강 설정 (건수 제외)</option></select></div>
              </div>
              <div className="space-y-1"><label className="text-xs font-black text-gray-400">과목 선택</label><select className="w-full px-4 py-3 bg-gray-50 rounded-xl font-bold text-sm border-none outline-none" value={assignForm.course_id} onChange={e => setAssignForm({...assignForm, course_id: e.target.value})}><option value="">개인 일정 (과제 건수 제외)</option>{safeCourses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
              <div className="space-y-1"><label className="text-xs font-black text-gray-400">제목</label><input required disabled={assignForm.category === 'cancellation'} placeholder={assignForm.category === 'cancellation' ? "휴강은 제목이 자동 생성됩니다." : "제목을 입력하세요."} className="w-full px-4 py-3 bg-gray-50 rounded-xl font-bold text-sm border-none outline-none" value={assignForm.title} onChange={e => setAssignForm({...assignForm, title: e.target.value})} /></div>
              <div className="space-y-1"><label className="text-xs font-black text-gray-400">과제(일정) 내용</label><textarea className="w-full px-4 py-3 bg-gray-50 rounded-xl font-bold text-sm border-none outline-none min-h-[100px] resize-none" value={assignForm.description} onChange={e => setAssignForm({...assignForm, description: e.target.value})} placeholder="상세 내용을 입력하세요." /></div>
              <div className="space-y-2">
                <label className="text-xs font-black text-gray-700 flex items-center gap-2"><Sparkles size={16} className="text-indigo-500" /> 세부 체크리스트</label>
                {assignForm.sub_tasks.map((task, idx) => (
                  <div key={idx} className="flex gap-2"><input className="flex-1 px-4 py-2.5 bg-gray-50 rounded-xl text-xs font-bold border-none outline-none" value={task} onChange={(e) => { const nt = [...assignForm.sub_tasks]; nt[idx] = e.target.value; setAssignForm({...assignForm, sub_tasks: nt}); }} /><button type="button" onClick={() => setAssignForm({...assignForm, sub_tasks: assignForm.sub_tasks.filter((_, i) => i !== idx)})} className="text-gray-300"><Trash2 size={18}/></button></div>
                ))}
                <button type="button" onClick={() => setAssignForm({...assignForm, sub_tasks: [...assignForm.sub_tasks, '']})} className="w-full py-3 border-2 border-dashed border-gray-100 text-gray-400 rounded-xl text-[10px] font-black">+ 항목 추가</button>
              </div>
            </div>
