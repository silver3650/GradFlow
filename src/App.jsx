...
export default function App() {
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 flex flex-col md:flex-row overflow-hidden font-sans">
      ...
      {/* 📱 모바일 고정 헤더 (학사모 로고 및 고정 노출)[cite: 3] */}
      <header className="md:hidden fixed top-0 left-0 right-0 bg-white border-b border-gray-100 h-[56px] flex items-center justify-between px-5 z-[50] shadow-sm">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-lg font-black text-[#1a1f2c] tracking-tighter italic">GradFlow</span>
        </div>
      ...
      {/* 📱 슬림화된 모바일 하단 내비바 (높이 감소)[cite: 3] */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-50 flex justify-around items-center py-2 px-2 z-[50] shadow-[0_-4px_20px_rgba(0,0,0,0.04)]">
        {navItems.map((item) => (
          <button 
            key={item.id} 
            onClick={() => setActiveTab(item.id)} 
            className={`flex flex-col items-center justify-center min-w-[60px] py-1 transition-all ${activeTab === item.id ? 'text-[#6366f1]' : 'text-gray-300'}`}
          >
